...
# Un mot sur ZFS on Linux <a id="mot_sur_zfs"></a>
...
# Environnement de test <a id="environnement_test"></a>
...
## Visuel (http://asciiflow.com/) <a id="visuel"></a>
...
# Installer ZFS sur NAS (Debian Buster) <a id="install_zfs_sur_nas"></a>
```sh
...
# Créer pool NAS <a id="creer_pool_nas"></a>
```sh
...
# Test en dégradé + reconstruction <a id="test_degrade"></a>
```sh
...
# Clone nas2 vers bkp2 <a id="clone_nas2_vers_bkp2"></a>
```sh
...
# ZFS sur BKP + pool RAID1 de 1 disque unique (Debian Buster) <a id="zfs_sur_bkp_pool_r1_1_disque"></a>
```sh
...
# Importer le pool NAS en lecture seule <a id="importer_pool_nas"></a>
```sh
...
# Mise en place de Rsnapshot <a id="mise_en_place_rsnapshot"></a>
```sh
# En Résumé
root@BKP:~# zfs mount
pool_bkp                        /pool_bkp
...
# Premières sauvegardes sur BKP <a id="premiere_sauvegarde"></a>
```sh
...
# Attacher bkp2 (clone de nas2) à pool_bkp <a id="attacher_bkp2_pool_bkp"></a>
```sh
...
# Finaliser sauvegarde depuis NAS <a id="finaliser_sauvegarde_depuis_nas"></a>
```sh
# Démarrer NAS
...
# BONUS
## Changer l'identification d'un disque dans un pool (Path <-> ID <-> device ...) <a id="changer_identification_disque"></a>
```sh
...
## Ajouter les options ZFS importantes <a id="ajouter_option_zfs"></a>
```sh
...
## Changer le montage du pool ZFS <a id="changer_montage_zfs"></a>
```sh
...
l)) sur un RAID1 (miroir) ZFS de 2 disques qui sauvegarderait les données d'un serveur de fichier OpenMediaVault **NAS** sous Linux, lui aussi sur un RAID1 ZFS de 2 disques. Les serveurs NAS et BKP seront dans des lieux différents.
